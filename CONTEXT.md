...
- **Harden Only Where Pressured**: A principle for post-run system improvement — restrict changes to boundaries the run actually stressed, preventing preemptive over-engineering based on theory alone. Validated by the first fast-local-diagnostic run: the brief theorized "Contract Mismatch" but the run stressed only weakness-type and logic-trace authoring, so only those were candidates for hardening.

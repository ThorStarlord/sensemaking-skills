...
**Conflict detection defined but unhandled**: repo-sensemaker SKILL.md (lines 26-35) documents "Detect Conflicts: If user intent != codebase diagnosis, flag it" with diagnosis_conflict and escalation_recommended fields. However, workflow-planner has no escalation logic for these flags.

**Validators proven**: All 5 Level-3 validators (validate-plan.py, validate-prompt-handoff.py, validate-brief.py, validate-skill-improvement-plan.py, validate-usage-research-report.py) triggered and validated successfully in Phase 5 (docs/PHASE5_SKILL_INVOCATION.md lines 53-79).

**PRD lifecycle gap**: VERDICT-SUMMARY.md (lines 67-78) documents that docs-architecture produces prd.md claim but doesn't validate it. Root cause: Design issue - artifact produced in wrong workflow.

**Intent propagation working**: IMPLEMENTATION-COMPLETE-phases-0-5.md (lines 133-149) shows artifact contracts updated to require source_intent_ref across repository_sensemaking_brief, workflow_orchestration_plan, prd, issue_list, agent_brief.
...
`skill_maintenance_workflow` (if treating repo itself as the project)
...
- Use: skill_maintenance_workflow (chains usage-researcher → skill-maintainer)
...
Use this prompt to invoke `skill_maintenance_workflow` or `problem-framer` for follow-up work:

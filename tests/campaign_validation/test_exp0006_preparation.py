"""Preparation proofs for EXP-0006 empirical Skill qualification diagnostics.

The suite deliberately uses the repository's canonical Two-Lane digest
implementation. During preparation, zero digest placeholders cause the first CI
pass to reveal the canonical configuration_id/policy_digest in assertion output;
those values are then frozen and the exact-head suite must pass before the
campaign envelope can be presented for human approval.
"""
...
FRAMEWORK_SHA = "969e8eb47144ffdeb27a8d9df02b6a292586e842"
...
APPROVAL_AUDIT_ISSUE_NUMBER = 274
DIAGNOSTIC_CASE_ID = "D-001"
DIAGNOSTIC_USER_INTENT = (
    "I'm considering the next major development step for this repository. "
    "Please inspect the repository and tell me what the project actually is now, "
    "where the most consequential uncertainty or weakness is, and what I should "
    "investigate or decide next. Do not implement changes."
)
NOT_BEFORE = "2026-09-04T21:50:00+00:00"
NOT_AFTER = "2026-09-11T23:59:59+00:00"
CURRENT_TIME = "2026-09-05T12:00:00+00:00"
...
def test_exp0006_configuration_digest_is_frozen_by_canonical_implementation() -> None:
    ...
    assert config["configuration_id"] == configuration_id, (
        f"EXP0006_CONFIGURATION_ID={configuration_id}"
    )


def test_exp0006_policy_digest_is_frozen_by_canonical_implementation() -> None:
    policy = _parse("campaign-policy.yaml")
    policy_digest = compute_policy_digest(policy)
    assert policy["policy_digest"] == policy_digest, (
        f"EXP0006_POLICY_DIGEST={policy_digest}"
    )
    assert _read("campaign-policy.sha256").decode().strip() == policy_digest


def test_exp0006_policy_and_configuration_validate() -> None:
...
def test_exp0006_exact_pins_and_diagnostic_contract_are_frozen() -> None:
    policy = _parse("campaign-policy.yaml")
    config = _parse("configuration-identity.yaml")
    ...
    assert config["execution_parameters"] == {
        ...
        "diagnostic_user_intent": DIAGNOSTIC_USER_INTENT,
        "candidate_execution_prohibited": True,
        "holdout_access_prohibited": True,
    }


def test_exp0006_intended_connector_native_receipt_shape_validates_synthetically() -> None:
    policy_result = validate_campaign_policy(_read("campaign-policy.yaml"), CONTEXT)
    assert policy_result.valid
    policy = policy_result.value
    raw_policy = _parse("campaign-policy.yaml")
    reference = (
        "https://github.com/ThorStarlord/sensemaking-skills/issues/274"
        "#issuecomment-5600000000"
    )
    receipt = {
...
def test_exp0006_approval_template_is_nonoperative() -> None:
...
def test_exp0006_preparation_package_contains_no_execution_state() -> None:
...
def test_exp0006_candidate_and_holdout_execution_are_not_in_this_envelope() -> None:
    config = _parse("configuration-identity.yaml")
    params = config["execution_parameters"]
    assert params["candidate_execution_prohibited"] is True
    assert params["holdout_access_prohibited"] is True
    assert config["prompt_or_skill_revision"] == FRAMEWORK_SHA
    assert config["framework_sha"] == FRAMEWORK_SHA

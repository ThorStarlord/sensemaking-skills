"""Preparation proofs for the GitHub-durable EXP-0004 successor campaign.
...
contain no operative approval or attempt state, while a future results branch
is governed separately by the GitHub-durable state validator.
"""
...
CAMPAIGN_ID = "EXP-0004-stage1-auteur-github-connector-pilot"
FRAMEWORK_SHA = "15763f036a1434387d77076b0fad3f5a796241ce"
...
NOT_BEFORE = "2026-08-18T16:00:00+00:00"
NOT_AFTER = "2026-08-25T16:00:00+00:00"
CURRENT_TIME = "2026-08-19T16:00:00+00:00"
...
def test_exp0004_configuration_digest_is_frozen_by_canonical_implementation() -> None:
    ...
    assert config["configuration_id"] == configuration_id, (
        f"EXP0004_CONFIGURATION_ID={configuration_id}"
    )


def test_exp0004_policy_digest_is_frozen_by_canonical_implementation() -> None:
    policy = _parse("campaign-policy.yaml")
    policy_digest = compute_policy_digest(policy)
    assert policy["policy_digest"] == policy_digest, (
        f"EXP0004_POLICY_DIGEST={policy_digest}"
    )
    assert _read("campaign-policy.sha256").decode().strip() == policy_digest


def test_exp0004_policy_and_configuration_validate() -> None:
...
def test_exp0004_exact_pins_and_connector_native_probe_are_frozen() -> None:
    policy = _parse("campaign-policy.yaml")
    config = _parse("configuration-identity.yaml")
...
def test_exp0004_approval_template_is_nonoperative() -> None:
...
def test_exp0004_preparation_package_contains_no_execution_state() -> None:

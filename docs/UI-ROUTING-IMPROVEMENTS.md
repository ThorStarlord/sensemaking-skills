...
#### New Files
- **`skills/repo-sensemaker/references/ui-fog-signals.md`**
...
- **`skills/repo-sensemaker/SKILL.md`**
  ...
  - Referenced new `ui-fog-signals.md` registry
...
full-local-sensemaking Workflow
  ...
  │   └─ Evaluates UI signals from ui-fog-signals.md
...
1. **Explicit UI Signal Detection**
   - Uses checkable indicators from `ui-fog-signals.md`
...
| `skills/repo-sensemaker/references/ui-fog-signals.md` | NEW | Checkable UI fog signals registry |
...
- ✅ `ui_fog` reliably detected on UI-heavy repositories (uses ui-fog-signals.md)
...
> `ui-implementation-workflow` consume a `context_artifacts` bundle (problem frame,
> unknowns map, sensemaking brief, orchestration plan) produced by a prior diagnostic
> run. Run them after a diagnostic workflow in the same session, or let the diagnostic
> workflow auto-invoke them. A cold-start with no context fails fast at pre-flight.

### Default UI Routing (recommended)
```bash
...
# Diagnoses fog type first, then auto-invokes the right implementation workflow
```

### Explicit UI Diagnostic Path (after sensemaking context exists)
```bash
python scripts/workflow-runtime.py --workflow ui-diagnostic-workflow --mode guided_execution
# Use when you want to review the UI assessment before implementation
```

### Manual Routing Override (after sensemaking context exists)
```bash
...
> **Note on execution mode**: the runtime default is `yolo_execution` (gates bypassed,
> validators enforce safety). The examples above always pass `--mode guided_execution`
> so you review each step. Choose the mode deliberately rather than relying on the default.
...
- `skills/repo-sensemaker/references/ui-fog-signals.md` — What signals indicate UI fog

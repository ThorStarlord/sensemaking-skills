...
| lean-canvas | deprecated `lean-canvas` | `business_canvas` | `product_strategy` | ADAPT | 3 | REPOSITORY_QUALIFIED | pending | pending | Canvas is explicitly a hypothesis map; critical assumptions do not automatically create experimentation responsibility. |
| experiment-design | deprecated `experiment-design` | `experiment_plan` | `experimentation` | ADAPT | 4 | REPOSITORY_QUALIFIED | pending | pending | Canonical v2 plans consume experiment warrant, represent decision discrimination / total cost / proportional controls, cannot encode results, and do not imply execution authority; historical v1 plans remain valid. |
| ab-test-analysis | deprecated `ab-test-analysis` | `test_results` | `experimentation` | ADAPT | 4 | REPOSITORY_QUALIFIED | pending | pending | Analyzed conclusions require actual observation evidence; otherwise insufficient; extend/investigate recommendations do not self-authorize another experiment. |
...
| pricing | deprecated `pricing` | `pricing_model` | `commercial_strategy` | ADAPT | 4 | REPOSITORY_QUALIFIED | pending | pending | Pricing remains proposed; observed prices/economics require evidence; experiment proposals are conditional on Experiment Economy; no price-change authority is granted. |

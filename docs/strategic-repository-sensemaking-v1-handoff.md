...
The milestone starts before a bounded implementation task has been selected:
...
OWNER_DECISION / THESIS_REVIEW
        |
bounded responsibility only when warranted + authorized
```

The goal was not to productize a deterministic StrategicPlanner. The active
semantic agent remains responsible for path generation, comparison, and
strategic judgment.
...
Package A PR-head tree and integrated tree were identical:
`ef469773e865d081958a1b89a93fe3edad161fa4`.

Package B PR-head tree and integrated tree were identical:
`1fbdc3852767cca2af94a71bae807160364b100c`.

The terminal closeout PR carries the final currentness/integration receipt for
Package C. Do not mutate source after integration merely to copy transient run
identifiers into this file.

## 3. First-class artifact and Skill

The milestone adds the supported engineering Skill:
...
The artifact is registered in:

- the canonical artifact contract registry;
- the compatibility artifact registry;
- the canonical vocabulary and packaged vocabulary;
- the engineering Domain Pack;
- the Skill manifest/catalog;
- the Version 1.0 supported-Skill inventory.
...
Current repository evidence, owner-supplied intent, inference, and claim ceilings
...
The canonical synthesis guidance requires material path distinctness.
...
The Skill prefers 2–5 paths when multiple futures are materially plausible, but
allows one path when alternatives would be artificial or only implementation
detail.

Generic path shapes can broaden search, but path-category coverage is never a
completion criterion.

## 6. Qualitative comparison

Paths are compared through the existing Level-3 lenses:
...
The validator cannot determine which path is strategically correct.

## 7. Decision-changing uncertainty and Inquiry Policy

After path comparison, the agent asks which unresolved premise could materially
change the strategic disposition.

Inquiry Policy v0 then governs whether additional evidence is actually worth
obtaining and which source is correct:
...
The milestone composes with Policy Hierarchy Completion v0 rather than replacing
it.
...
`OWNER_DECISION` names owner intent/authority.
`THESIS_REVIEW` escalates the challenged Level-4 commitment.
`DEFER` and `NO_CHANGE` create no implementation task by default.

## 9. Diagnostic versus strategic repository sensemaking

The product now distinguishes two repository-level surfaces:
...
A current `repository_sensemaking_brief` may be reused by strategic analysis,
but is not mandatory when the strategic Skill can establish current state
directly from the authorized repository evidence surface.
...
`using-sensemaking` now routes open repository-future questions into
`strategic-repository-analysis`.

README and Getting Started expose the same distinction without requiring users
to know the internal Level-3 vocabulary.

The user can ask, in ordinary language:
...
`scripts/validate-strategic-repository-analysis.py` checks representation:

- required machine fields;
- capability-state vocabulary;
- frontier/path identifier integrity;
- path field shape;
- complete qualitative comparison lenses;
...
Mechanical PASS therefore establishes artifact integrity only.
...
- Campaign schema v3;
- generic `AgentState`;
- a generic cognition/state database;
...
## 13. Relationship to experimental StrategicPlanner work

Issue #395 remains historical lab evidence at `RESEARCH_MORE`.

Strategic Repository Sensemaking v1 does not promote that experimental planner
into the product. The v1 capability is a structured semantic-agent surface, not
...
Issue #399 remains separately owner-authorized.

Inquiry Policy v0 is already integrated and composes directly with Strategic
Repository Sensemaking. Later Policy Hierarchy packages remain separate bounded
responsibilities.

Closing Issue #401 does not automatically select or start the next #399 package.
...
- that this method is superior to unaided reasoning;
- native-harness usefulness;
- independent-harness portability;
- product-market value;
- that any selected construction path will succeed.

No experiment was required to make the owner-directed product capability exist.

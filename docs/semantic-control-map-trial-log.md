...
| 2026-09-02 | MECH refresh (trigger: `test_path_drift.py` / `test_cli.py` red/green state — both GREEN on `main`, same CI evidence as SE2; flagged by R2 as its F2) | SA10, SA12 | ~2 | dispatcher context (campaign R2 close-out audit). SA10 enforcement-state cell and SA12 enforcer/impl-vs-policy cells corrected; ids/Grade/Rate/Deriv unchanged; no rows added. Also corrected the protocol's step-4 test selector (`tests/test_cli.py::test_cli_version` no longer resolves; now `TestCLIBasic::test_cli_version`) — mechanical, no semantic change to the protocol |

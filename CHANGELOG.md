...
### Changed
- **Strategic Front Door Productization v1** — declares `strategic-sensemaking-loop` as a Version 1.0 public agent entrypoint while keeping it semantically `internal`; the release contract now distinguishes public compositional entrypoints from manifest-backed independent semantic Skills, validates canonical/non-experimental entrypoint identity, and preserves the existing no-master-artifact, no-planner, native-harness, portability, and semantic-usefulness claim ceilings.

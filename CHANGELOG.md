...
### Changed
- **Strategic Sensemaking normal-use observation guide** — turns the current normal-use validation posture into a lightweight, reusable observation discipline for breadth before convergence, frontier-candidate compression, proportional depth, semantic resume, BUILD-versus-unnecessary-inquiry, and Goal Fitness / qualification-frontier integrity; extends the existing normal-use evidence lane and explicitly adds no score, benchmark, experiment program, schema, tracker, runtime, or automatic strategy reopening.
- **Autonomous Terminal Mission Continuation v1 (Issue #473)** — extends `strategic-sensemaking-loop` with an explicit FULL AUTONOMY / FULL DELEGATION terminal-mission profile that continues across bounded responsibilities until the authoritative outcome is satisfied or a genuine stop boundary is reached; keeps Strategic Exploration Funnel breadth conditional on open/reopened Level 3, avoids forced three-path search and blind backlog execution, completes only decision-relevant vertical layers, supports mission-scoped construction-before-field-validation with synthetic-persona claim discipline, gates canonical/status promotion on verification + reconciliation, and preserves separately governed merge/release/deploy/Level-4/owner-preference authority. Adds no new top-level Skill, planner, permission engine, numeric optimizer, Campaign schema, or automatic protected-transition authority.
- **Strategic Exploration Funnel v1 (Issue #468)** — makes Level-3 repository exploration explicitly breadth→depth: map major product/control systems, scan within/across systems for deficiencies and opportunities, synthesize grounded frontier candidates, depth-drill only material finalists, and synthesize construction paths only after frontier depth; `strategic-sensemaking-loop` exposes a compact Strategic Exploration Summary when `ANALYZE/REOPEN_ANALYSIS` actually runs while explicitly skipping repository-wide breadth reanalysis during settled responsibility/execution/verification/reconciliation. No new artifact schema, numeric scoring, deterministic search tree/router, exhaustive file inventory, or mandatory opportunity-per-system behavior is added.
- **Value-Producing Action & High-Delegation v1 (Issue #465)** — strengthens the existing reversible-build/commission-omission guidance into a qualitative preference for the highest-value warranted and authorized action, adds a repository-scoped high-delegation envelope with protected-transition boundaries, allows independently warranted work to continue around a blocked gate without bypassing it, and makes consequential Level-3 alternatives/value visible through a compact Decision Trace; no numeric expected-value score, automatic action router, mandatory reversible build/experiment, permission engine, or merge/release authority expansion is added.
- **Strategic Sensemaking Loop Organization reconciliation (Issue #462)** — reconciles integrated Capability & Organization Tracer v0 evidence as `REAFFIRM`, adds optional read-only `organization inspect|role|skill-profile` guidance inside the existing `EXECUTE` boundary, and preserves `Organization Pattern != execution authority`, `role binding != actor allocation`, no mandatory Organization stage, no automatic Skill routing, and no worker/runtime expansion.
- **Capability & Organization Tracer v0 (Issue #459)** — adds a shipped read-only `organization` CLI surface, a non-authoritative Skill-capability facet overlay, and the first explicit Repository Change Cell role topology (Controller/optional Analyst/Builder/Verifier/Reconciler); it validates declared capability coverage and evidence-flow references while reusing the existing external execution boundary and adding no scheduler, worker pool, automatic role assignment, Skill router, Campaign schema change, or execution authority.
- **Strategic Sensemaking Loop v1 (Issue #452)** — adds the internal `strategic-sensemaking-loop` one-prompt front door for starting or resuming strategic repository episodes from durable artifacts; it conditionally composes strategic analysis, responsibility selection, real execution surfaces, returned-evidence reconciliation, and reserved-decision packets, skips already-completed stages, and adds no master artifact, deterministic planner/router, Campaign schema change, mandatory handoff choreography, or authority expansion.
- **Goal Fitness & Frontier Integrity v1 (Issue #449)** — prevents milestone/proxy inversion in `strategic-repository-analysis` by classifying objective role, testing goal fit against grounded governing intent, checking decision-relevant completion layers before downstream qualification/release frontier selection, and challenging path sets that share one downstream frame; adds a narrow inherited-responsibility prerequisite backstop in `using-sensemaking` and makes `owner-decision-capsule` fail closed on materially incomplete option sets without schema v3, numeric scoring, mandatory third paths, or a new runtime.
- **Strategic Hypothesis Admission v1 (Issue #446)** — corrects future-state evidence asymmetry in `strategic-repository-analysis`: present-state facts still require evidence, while coherent unbuilt futures may enter as grounded strategic hypotheses; opportunity-driven frontier admission, commission/omission symmetry, and reversible build-as-learning become explicit without schema v3, a new disposition, automatic path selection, or weakened implementation authority.
- **Experiment Responsibility Boundary v1 (Issue #441)** — diagnostic/domain Skills no longer manufacture experimentation responsibility from empirical uncertainty; Experiment Economy retains warrant selection; experiment-design now consumes warrant and emits backward-compatible canonical `experiment_plan` v2 with decision discrimination, qualitative total cost, proportional controls, and claim ceiling; analysis continuation recommendations require fresh warrant.
- **Experiment Economy & Proportional Rigor v1 (Issue #438)** — separates inquiry from experimentation, requires Experiment Warrant before experimental work, treats cheap reversible construction as a possible evidence source, counts total experiment overhead, applies minimum sufficient rigor and Confounder Warrant, and distinguishes normal-use product evidence from causal/research-grade evidence without adding experiment automation or scoring.
- **Strategic Repository Analysis Semantic Grounding v1 (Issue #435)** — adds the Strategicity Gate before Level-3 frontier/path formation, backward-compatible `schema_version: 2` grounding for frontier/path evidence and capability references, required path plausibility, and strategy-before-intervention ordering while preserving versionless v1 artifacts, semantic-agent ownership, and non-authoritative mechanical validation.

...
_PM_FEATURE_DEFINITION_IDS = {"story_list", "criteria_list", "risk_analysis"}
_PM_STRATEGY_IDS = {
    "market_analysis",
    "strategy_doc",
    "prioritized_list",
...
_PM_MEASUREMENT_IDS = {"experiment_plan", "test_results", "pmf_report", "pricing_model"}
_PM_CUSTOMER_MODEL_IDS = {"journey_map", "ideal_customer_profile"}
...
def extract_artifact_id_detailed(artifact_path: str) -> ArtifactIdExtraction:
    ...
    try:
        ...
        return ArtifactIdExtraction("no_authoritative_block", detail=f"could not read file: {exc}")
...
_ROUTING_ERROR_TAXONOMY = {
    ...
    "ambiguous_authoritative_blocks": (
        ...
        ["Remove duplicate machine-readable handoff sections"],
    ),
    "malformed_authoritative_yaml": (
        "malformed_authoritative_yaml",
        "The authoritative machine-readable handoff block's YAML could not be parsed and no artifact_id could be recovered.",
        ["Fix the YAML syntax error in the authoritative handoff block"],
    ),
    "missing_field": (
        "missing_field",
        "Cannot determine artifact_id from the authoritative machine-readable handoff block.",
        ["Add artifact_id to the authoritative machine-readable handoff YAML block"],
    ),
    "duplicate_key": (
        "duplicate_key",
        "artifact_id appears more than once in the authoritative handoff block.",
        ["Remove the duplicate artifact_id key"],
    ),
    "conflicting_artifact_id": (
        "conflicting_value",
        "artifact_id appears more than once with conflicting values.",
        ["Resolve conflicting artifact_id values"],
    ),
}
...
def _routing_error_result(status: str, detail: str, artifact_path: str) -> ValidationResult:
    error_suffix, message, suggested_fixes = _ROUTING_ERROR_TAXONOMY.get(
        status,
        ("missing_field", "Cannot determine artifact_id from file.", ["Add artifact_id field"]),
    )
...
def invoke_validator(
    ...
    try:
        if validator_path == "scripts/validate-artifact.py":
            ...
            cmd = [sys.executable, validator_path, artifact_id, artifact_path, "--repo-root", repo_root, "--json"]
        else:
            cmd = [sys.executable, validator_path, artifact_path, "--repo-root", repo_root, "--json"]
        ...
        try:
            ...
            return {
                ...
                "errors": [{
                    ...
                    "suggested_fixes": ["Run the validator directly with --json", "Check validator stderr"],
        ...
        return {
            ...
            "errors": [{
                ...
                "suggested_fixes": ["Ensure validator script exists"],
                "reference": "docs/validator-json-refactor-guide.md",
            }],
            "validation_timestamp": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate any Phase 1 artifact and return unified structured JSON.")
    ...
    parser.add_argument("--target-repo", default=None, help="Root of the repository the artifact is ABOUT")
    parser.add_argument("--probe-report", default=None, help="Same-episode local Probe Engine report path")
    args = parser.parse_args(argv)
    result = validate_and_report(args.artifact_path, args.repo_root, args.target_repo, args.probe_report)
